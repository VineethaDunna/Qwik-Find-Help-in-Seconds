...
const saveAuthToLocal = (data: any) => {
	try {
		...
		console.warn("saveAuthToLocal failed:", err);
	}
};
...
const Index = () => {
	...
	// EMAIL / PASSWORD LOGIN (uses AuthContext.login which refreshes user)
	const handleSubmit = async (e: React.FormEvent) => {
		e.preventDefault();
		setError(null);
		if (!email || !password) return toast("Email and password required");
		...
		try {
			// Use login() from AuthContext which calls loginApi + refreshUser()
			...
			// login -> AuthContext.refreshUser() should have run and set user
	...
	const handleGoogleCredential = async (credentialResponse: any) => {
		...
		try {
			...
			saveAuthToLocal(json.data ?? json);

			// Ensure AuthContext has fresh user data
			try {
				await refreshUser();
			} catch (err) {
				console.warn("refreshUser after Google login failed", err);
			}
	...
	// manual click fallback: prompt one-tap
	const handleGoogleClick = () => {
		setError(null);
		if (!(window as any).google?.accounts?.id)
			return toast("Google SDK not ready");
			...
			<main className='container py-8 md:py-12'>
				<div className='mx-auto max-w-md animate-fade-in-up'>
					...
					<div className='bg-card rounded-2xl border border-border p-6 shadow-card'>
						...
						<div id='gsi-button' className='mb-4' />
