...
interface WorkerWorkboardProps {
	// raw backend bookings (usually confirmed ones)
	bookings: any[];
...
const WorkerWorkboard = ({ bookings, onComplete }: WorkerWorkboardProps) => {
	...
	const todayJobs = sortedBookings.filter((b) => isToday(getDate(b)));
	const tomorrowJobs = sortedBookings.filter((b) => isTomorrow(getDate(b)));
	const upcomingJobs = sortedBookings.filter((b) => {
		const days = differenceInDays(getDate(b), new Date());
		return days > 1;
	});
	...
	const JobCard = ({
		...
		const price =
			...
			(booking.worker?.hourly_rate || 0) * (booking.duration_hours || 1);
	...
	const EmptyState = () => (
		<div className='text-center py-12'>
			...
			<p className='text-sm text-muted-foreground mt-1'>
				Accept bookings to see them here
			</p>
		</div>
	);
	...
	return (
		<div className='space-y-6'>
			...
			<div className='grid grid-cols-3 gap-3'>
				<Card
					className={
						todayJobs.length > 0 ? "bg-accent/10 border-accent/20" : ""
					}>
				...
				<>
					{todayJobs.length > 0 && (
						<div className='space-y-3'>
							...
							{todayJobs.map((b) => (
								<JobCard key={b.id} booking={b} urgent />
							))}
						</div>
					)}
